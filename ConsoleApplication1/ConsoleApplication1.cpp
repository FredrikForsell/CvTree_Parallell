...
void Init()
{
	...
	for (int i = 0; i<omp_get_num_threads(); i++)
		...
		*/
		...
		//Due to memory constrictions this cannot be parallelized.
...
public:
	...
	Bacteria(char* filename)
	{
		...
		for (int i = 0; i<M1; i++)
			second_div_total[threadNumber][i] = (double)second[threadNumber][i] / total_plus_complement;
	...
	//	double* t = new double[M]; // move to global
		...
		{
			double p1 = second_div_total[threadNumber][i_div_aa_number]; //TODO:: INITIATE INSIDE LOOP CAUSING MORE WORK?
			double p2 = one_l_div_total[i_mod_aa_number];
			double p3 = second_div_total[threadNumber][i_mod_M1];
